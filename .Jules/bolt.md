...
## 2025-02-12 - [Client-Side Promise Caching for Deduplicating Fetch]

**Learning:** Client-side sibling components like TopArtists and TopGenres that fetch the same endpoint simultaneously trigger duplicate network requests, leading to redundant backend processing.
**Action:** Implement an in-memory client-side Promise cache (e.g., using a Map) to deduplicate concurrent fetch requests across frontend components to optimize performance.
