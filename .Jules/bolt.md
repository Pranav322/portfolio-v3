...
## 2025-02-12 - [Client-Side Deduplication of Sibling API Requests]

**Learning:** Client-side sibling components (like TopArtists and TopGenres) fetching the same endpoint simultaneously on mount trigger redundant duplicate network requests.
**Action:** Implement an in-memory client-side Promise cache (e.g., `deduplicatedFetchJSON`) to deduplicate concurrent fetch requests across frontend components.
