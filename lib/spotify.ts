...
// Cache variables for Spotify access token optimization
...
async function getAccessToken() {
  ...
  // Return cached promise if we have one and the token hasn't expired
  // Check explicit pending state (=== 0) to avoid concurrent requests bypassing cache
  if (cachedTokenPromise && (tokenExpirationTime === 0 || now < tokenExpirationTime)) {
    return cachedTokenPromise;
  }

  // Explicitly set pending state before fetching
  ...
  cachedTokenPromise = fetch(TOKEN_ENDPOINT, {
    ...
    .then(async response => {
      // Explicitly reject non-ok responses to prevent silently caching errors
      if (!response.ok) {
        throw new Error(`Spotify token fetch failed: ${response.statusText}`);
      }
      const data = await response.json();
      // Cache the token for slightly less than its actual expiration (usually 3600s -> 3500s)
      tokenExpirationTime = now + (data.expires_in - 100) * 1000;
      return data;
    })
    .catch(error => {
      // Reset cache state on error to avoid permanently poisoning the cache
